...
class SessionControllerTest {
    ...
    @Test
    void session_productionBackend_doesNotExposeSettingsDeepLink() throws Exception {
        String token = TestJwtForger.forgeInstalledToken();
        mockMvc.perform(get("/api/session").header("X-Addon-Token", token))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.settingsUrl").doesNotExist());
    }

    @Test
    void session_devPortalBackend_doesNotExposeSettingsDeepLink() throws Exception {
